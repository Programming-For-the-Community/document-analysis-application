...
export function validateSignUp(username: string, password: string, confirmPassword: string): string | null {
  if (!username || !password || !confirmPassword) return loginErrors.MISSING_SIGNUP_FIELDS;
  if (password !== confirmPassword) return loginErrors.PASSWORDS_DO_NOT_MATCH;
  if (password.length < 8) return loginErrors.PASSWORD_TOO_SHORT;
  return null;
}
...
export async function signUp(username: string, password: string): Promise<{ error: string | null }> {
  ...
  return result.success ? { error: null } : { error: result.error ?? loginErrors.SIGNUP_FAILURE };
}