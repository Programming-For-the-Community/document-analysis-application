...
export function useNewProject() {
  ...
  const open  = useCallback(() => { setError(null); setIsOpen(true); }, []);
  const close = useCallback(() => { setIsOpen(false); setError(null); }, []);

  const submit = useCallback(async (name: string) => {
    if (!name.trim()) { setError(HOME_ERRORS.PROJECT_NAME_MISSING); return; }
    setSubmitting(true);
    const { project, error: err } = await createProject(name.trim());
    setSubmitting(false);
    if (err || !project) { setError(err ?? HOME_ERRORS.PROJECT_CREATION_FAILURE); return; }
    close();
