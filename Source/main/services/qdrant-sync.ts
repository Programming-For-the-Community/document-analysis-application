  ...
  try {
    ...
    for (const project of projects) {
      try {
        ...
        if (missing.length === 0) {
          Logger.debug(
            `Qdrant sync: project "${project.name}" — all ${complete.length} document(s) already in Qdrant`
          );
          continue;
        }

        Logger.info(
          `Qdrant sync: project "${project.name}" — loading ${missing.length}/${complete.length} missing document(s)`
        );
          ...
          complete
            ...
            .map(async (doc) => {
              ...
              try {
                ...
                Logger.error(
                  `Qdrant sync: failed for "${doc.documentName}" (${doc.documentId}): ${message}`
                );
              }
            })
        );
        ...
        const message = err instanceof Error ? err.message : String(err);
        Logger.error(
          `Qdrant sync: error processing project "${project.name}" (${project.id}): ${message}`
        );
      }
    }
