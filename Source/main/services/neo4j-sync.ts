  ...
  try {
    ...
    for (const project of projects) {
      try {
        ...
        if (missing.length === 0) {
          Logger.debug(`Neo4j sync: project "${project.name}" — all ${complete.length} document(s) already in graph`);
          continue;
        }

        Logger.info(`Neo4j sync: project "${project.name}" — loading ${missing.length}/${complete.length} missing document(s)`);
          ...
          complete
            ...
            .map(async (doc) => {
              ...
              try {
                ...
                Logger.error(`Neo4j sync: failed to load "${doc.documentName}" (${doc.documentId}): ${message}`);
              }
            })
        );
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        Logger.error(`Neo4j sync: error processing project "${project.name}" (${project.id}): ${message}`);
      }
    }
