...
export function registerProjectHandlers(
  ...
  ipcMain.handle(
    'project:create',
    async (_event, projectName: string): Promise<ProjectCreateResult> => {
      const config = getAppConfig();
      const tokens = getTokens();
  ...
  ipcMain.handle(
    'project:rename',
    async (_event, projectId: string, newName: string): Promise<ProjectRenameResult> => {
      const config = getAppConfig();
      const tokens = getTokens();
  ...
  ipcMain.handle(
    'project:delete',
    async (_event, projectId: string): Promise<ProjectDeleteResult> => {
      const config = getAppConfig();
      const tokens = getTokens();
      ...
      try {
        ...
        await Neo4J.deleteProject(projectId).catch((err: unknown) =>
          Logger.warn(
            `project:delete Neo4j cleanup failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`
          )
        );
        await Qdrant.deleteProject(projectId).catch((err: unknown) =>
          Logger.warn(
            `project:delete Qdrant cleanup failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`
          )
        );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const existingRole = await AWS_DYNAMODB.getProjectRole(
          targetUser.sub,
          projectId,
          config.dynamoDB
        );
        ...
        if (existingRole) {
          return {
            success: false,
            error: `"${targetUsername}" already has access to this project`,
          };
        }

        await AWS_DYNAMODB.shareProject(
          projectId,
          targetUser.sub,
          targetUser.username,
          role,
  ...
  ipcMain.handle('project:list-members', async (_event, projectId: string) => {
    ...
    try {
      ...
      Logger.error(
        `project:list-members error: ${err instanceof Error ? err.message : String(err)}`
      );
      return { success: false, error: message };
    }
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        Logger.error(
          `project:search-users error: ${err instanceof Error ? err.message : String(err)}`
        );
        return { success: false, error: message };
      }
    }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        Logger.error(
          `project:get-settings error: ${err instanceof Error ? err.message : String(err)}`
        );
  ...
  ipcMain.handle(
    'project:set-min-doc-filter',
    async (_event, projectId: string, minDocFilter: number): Promise<SimpleResult> => {
      const config = getAppConfig();
      const tokens = getTokens();
      if (!config || !tokens || typeof tokens === 'boolean')
        return { success: false, error: 'App not ready' };
      try {
        ...
        Logger.error(
          `project:set-min-doc-filter error: ${err instanceof Error ? err.message : String(err)}`
        );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        Logger.error(
          `project:update-role error: ${err instanceof Error ? err.message : String(err)}`
        );
        return { success: false, error: message };
      }
    }
