...
export function registerUserHandlers(
  ...
  ipcMain.handle('user:get-preference', async (_event, key: string): Promise<{ success: boolean; value?: string | null; error?: string }> => {
    ...
    try {
      ...
      Logger.error(`user:get-preference error: ${err instanceof Error ? err.message : String(err)}`);
      return { success: false, error: 'Failed to get preference' };
    }
  });

  ipcMain.handle('user:set-preference', async (_event, key: string, value: string): Promise<{ success: boolean; error?: string }> => {
    const config = getAppConfig();
    const tokens = getTokens();
    if (!config || !tokens || typeof tokens === 'boolean') return { success: false, error: 'App not ready' };
    try {
      ...
      Logger.error(`user:set-preference error: ${err instanceof Error ? err.message : String(err)}`);
